...
const HomeIcon = () => (
  <svg viewBox="0 0 24 24" aria-hidden="true">
    <path d="M3 10.8 12 3l9 7.8v9.1a1.1 1.1 0 0 1-1.1 1.1h-5.2v-6.2H9.3V21H4.1A1.1 1.1 0 0 1 3 19.9v-9.1Z" />
  </svg>
);
...
export const Navbar = () => {
  ...
  const avatarUrl =
    currentProfile?.avatar_url?.trim() || user?.user_metadata?.avatar_url || null;
    ...
    <>
      <header className="yapster-navbar">
        <div className="yapster-navbar__inner">
          <Link to="/" className="yapster-brand" aria-label="Yapster home">
            <img src="/yapster-mark.svg" alt="" className="yapster-brand__mark" />
            <span className="yapster-brand__word">Yapster</span>
          </Link>
          ...
          <nav className="yapster-desktop-nav" aria-label="Main navigation">
            {navItems.slice(0, 2).map((item) => (
              <Link
                key={item.label}
                to={item.to}
                className={`yapster-nav-link ${isActive(item.to) ? "is-active" : ""}`}
              >
                {item.label}
              </Link>
            ))}
          ...
          <Link to="/search" className="yapster-search-trigger" aria-label="Search Yapster">
            <SearchIcon />
            <span>Search communities, posts, and people</span>
            <kbd>/</kbd>
          </Link>
          ...
          <div className="yapster-navbar__actions">
            ...
            {user ? (
              <>
                ...
                <button
                  type="button"
                  className="yapster-profile-trigger"
                ...
                >
                  {avatarUrl ? (
                    ...
                    <span>{profileUsername.slice(0, 1).toUpperCase()}</span>
                  )}
                </button>
              </>
            ) : (
              <div className="yapster-guest-actions">
                <Link to="/login" className="yapster-button yapster-button--ghost">
                  Log in
                </Link>
                <Link to="/signup" className="yapster-button yapster-button--primary">
                  Join Yapster
                </Link>
              </div>
            )}
          ...
          {user && menuOpen && (
            <div className="yapster-account-menu">
              <div className="yapster-account-menu__identity">
              ...
              <Link to="/communities" onClick={() => setMenuOpen(false)}>
                My communities
              </Link>
      ...
      <nav className="yapster-mobile-nav" aria-label="Mobile navigation">
        ...
        <Link to="/communities" className={isActive("/communities") ? "is-active" : ""}>
          <CompassIcon />
          <span>Explore</span>
        </Link>
        <Link to={user ? "/create" : "/login"} className="yapster-mobile-create" aria-label="Create post">
          <PlusIcon />
        </Link>
        <Link to="/search" className={isActive("/search") ? "is-active" : ""}>
          <SearchIcon />
          <span>Search</span>
        </Link>
        <Link to={user ? profileHref : "/login"} className={location.pathname.startsWith("/profile") ? "is-active" : ""}>
          <UserIcon />
