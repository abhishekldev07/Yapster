    ...
    <div className="space-y-5">
      ...
      <div className="yapster-card p-3 sm:p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          ...
          <label className="min-w-[220px] text-xs font-extrabold text-slate-500"><span className="sr-only">Filter by category</span><select value={category} onChange={(event) => setCategory(event.target.value)} className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-normal text-slate-700 outline-none focus:border-violet-300 focus:bg-white"><option>All categories</option>{COMMUNITY_CATEGORIES.map((item) => <option key={item}>{item}</option>)}</select></label>
          <span className="px-1 text-xs font-semibold text-slate-400">{filteredCommunities.length} {filteredCommunities.length === 1 ? "community" : "communities"}</span>
        </div>
        <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3"><span className="mr-1 text-[11px] font-extrabold uppercase tracking-[0.12em] text-slate-400">Sort</span>{([["trending","Trending"],["new","Newest"],["largest","Largest"]] as [CommunitySort,string][]).map(([value,label]) => <button key={value} type="button" onClick={() => setSort(value)} aria-pressed={sort === value} className={`rounded-lg px-3 py-1.5 text-xs font-extrabold transition ${sort === value ? "bg-slate-950 text-white" : "text-slate-500 hover:bg-slate-100 hover:text-slate-900"}`}>{label}</button>)}</div>
      </div>
      ...
      {filteredCommunities.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filteredCommunities.map((community) => {
        ...
        return <article key={community.id} className="group relative overflow-hidden rounded-[18px] border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_14px_34px_rgba(15,15,25,0.08)]">
          ...
          <div className="p-4 sm:p-5"><div className="flex items-start justify-between gap-3"><Link to={`/community/${community.id}`} className="flex min-w-0 items-center gap-3"><span className="yapster-community-initial grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-[13px] bg-gradient-to-br from-orange-100 via-pink-100 to-violet-100 text-sm font-black text-violet-800 ring-1 ring-black/5">{community.icon_url ? <img src={community.icon_url} alt="" className="h-full w-full object-cover" /> : cleanName.slice(0,1).toUpperCase()}</span><span className="min-w-0"><strong className="block truncate text-base font-extrabold text-slate-950 transition group-hover:text-violet-700">{cleanName}</strong><span className="mt-0.5 block truncate text-[10px] font-extrabold uppercase tracking-[0.1em] text-violet-600">{community.category || "Other"}</span></span></Link>
          {isOwner ? <span className="shrink-0 rounded-lg border border-amber-200 bg-amber-50 px-3 py-1.5 text-xs font-extrabold text-amber-800">Admin</span> : <button type="button" disabled={authLoading || isPending || joinOrLeaveMembership.isPending} onClick={() => { if (!user) return void signInWithGitHub(); setPendingCommunityId(community.id); joinOrLeaveMembership.mutate({ communityId: community.id, isJoined: isMember }); }} className={`shrink-0 rounded-lg px-3 py-1.5 text-xs font-extrabold transition ${isMember ? "border border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100" : "border border-violet-600 bg-violet-600 text-white hover:bg-violet-700"} disabled:opacity-60`}>{isPending ? "Working..." : isMember ? "Joined" : "Join"}</button>}</div>
          <p className="mt-4 min-h-10 line-clamp-2 text-sm leading-5 text-slate-500">{community.description || "A Yapster community."}</p><div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-3 text-[11px] font-semibold text-slate-400"><span>{memberCount} {memberCount === 1 ? "member" : "members"}</span><span>{recentActivity > 0 ? `${recentActivity} recent activities` : "Open community"}</span></div><Link to={`/community/${community.id}`} className="mt-3 inline-flex items-center gap-1 text-xs font-extrabold text-violet-700">Open <ArrowIcon /></Link></div>
        </article>;
      })}</div> : <div className="yapster-card p-9 text-center"><h3 className="text-lg font-black text-slate-950">No communities match</h3><p className="mt-2 text-sm text-slate-500">Try another category or search term.</p></div>}
    </div>
  );
};
