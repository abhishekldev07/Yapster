...
interface CommunitySummary { id: number; name: string; }
interface AuthorSummary { username: string | null; display_name: string | null; avatar_url: string | null; }

const fetchPostById = async (id: number): Promise<PostWithCommunity> => {
  const { data, error } = await supabase
    .from("posts")
    .select("id, user_id, community_id, title, content, created_at, image_url, post_type, link_url, flair_id, avatar_url, post_flairs(id, name, color)")
    .eq("id", id)
    .single();
  ...
  return {
    ...
    flair_id: row.flair_id != null ? Number(row.flair_id) : undefined,
    flair_name: row.post_flairs?.name ?? undefined,
    flair_color: row.post_flairs?.color ?? undefined,
  } as PostWithCommunity;
};

const CommentIcon = () => (
  <svg viewBox="0 0 24 24" className="h-4 w-4 fill-none stroke-current stroke-[1.8]" aria-hidden="true"><path d="M20 11.5a7.7 7.7 0 0 1-8 7.5 8.8 8.8 0 0 1-3-.5L4 20l1.4-4A7.3 7.3 0 0 1 4 11.5 7.7 7.7 0 0 1 12 4a7.7 7.7 0 0 1 8 7.5Z" strokeLinecap="round" strokeLinejoin="round" /></svg>
);
const ShareIcon = () => (
  <svg viewBox="0 0 24 24" className="h-4 w-4 fill-none stroke-current stroke-[1.8]" aria-hidden="true"><path d="M8 12 16.5 5M12.5 5h4v4M18 13v5a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1V7a1 1 0 0 1 1-1h5" strokeLinecap="round" strokeLinejoin="round" /></svg>
);
const LinkIcon = () => (
  <svg viewBox="0 0 24 24" className="h-5 w-5 fill-none stroke-current stroke-[1.8]" aria-hidden="true"><path d="M9.5 14.5 14.5 9M7.7 16.3l-1.3 1.3a3.5 3.5 0 0 1-5-5l3-3a3.5 3.5 0 0 1 5 0M16.3 7.7l1.3-1.3a3.5 3.5 0 1 1 5 5l-3 3a3.5 3.5 0 0 1-5 0" strokeLinecap="round" /></svg>
);
...
export const PostDetail = ({ postId }: Props) => {
  ...
  const { data: community } = useQuery<CommunitySummary | null, Error>({
    ...
    queryFn: async () => {
      const { data: communityData, error: communityError } = await supabase.from("communities").select("id, name").eq("id", data!.community_id!).maybeSingle();
  ...
  const { data: author } = useQuery<AuthorSummary | null, Error>({
    ...
    queryFn: async () => {
      const { data: profile, error: profileError } = await supabase.from("profiles").select("username, display_name, avatar_url").eq("id", data!.user_id!).maybeSingle();
      if (profileError) throw new Error(profileError.message);
      return profile as AuthorSummary | null;
    },
  });
  ...
  const { data: commentCount = 0 } = useQuery<number, Error>({
    ...
    queryFn: async () => {
      const { count, error: commentsError } = await supabase.from("comments").select("id", { count: "exact", head: true }).eq("post_id", postId);
  ...
  const handleShare = async () => {
    ...
    try {
      if (navigator.share) await navigator.share({ title: data?.title, text: data?.content || undefined, url });
      else await navigator.clipboard.writeText(url);
      setShareState(navigator.share ? "Shared" : "Link copied");
      window.setTimeout(() => setShareState(null), 1800);
    } catch { setShareState(null); }
  };

  if (isLoading) return <div className="yapster-card animate-pulse p-6 sm:p-8"><div className="h-3 w-40 rounded bg-slate-100" /><div className="mt-5 h-9 w-4/5 rounded bg-slate-100" /><div className="mt-5 h-3 w-full rounded bg-slate-100" /><div className="mt-2 h-3 w-5/6 rounded bg-slate-100" /><div className="mt-6 h-64 rounded-2xl bg-slate-100" /></div>;
  ...
  const typeLabel = data.flair_name || (data.post_type === "image" ? "Image" : data.post_type === "link" ? "Link" : data.post_type === "poll" ? "Poll" : "Discussion");
    ...
    <article className="overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-sm">
      ...
      <div className="p-5 sm:p-8">
        <div className="flex items-start justify-between gap-4">
          ...
          <span className="rounded-full px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-[0.12em] text-violet-700" style={data.flair_color ? { backgroundColor: `${data.flair_color}20`, color: data.flair_color } : { backgroundColor: "#f5f3ff" }}>{typeLabel}</span>
        </div>
