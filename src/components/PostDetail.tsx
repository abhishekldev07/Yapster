      ...
      <article className="overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-sm">
        ...
        <div className="p-5 sm:p-8">
          <div className="flex items-start justify-between gap-4">
            <div className="flex min-w-0 items-center gap-3"><Link to={community ? `/community/${community.id}` : "/communities"} className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-[13px] bg-gradient-to-br from-orange-100 via-pink-100 to-violet-100 text-sm font-bold text-violet-800 ring-1 ring-black/5">{community?.icon_url ? <img src={community.icon_url} alt="" className="h-full w-full object-cover" /> : (community?.name || "C").trim().slice(0,1).toUpperCase()}</Link><div className="min-w-0">{community ? <Link to={`/community/${community.id}`} className="block truncate text-sm font-semibold text-slate-900 hover:text-violet-700">{community.name.trim()}</Link> : <span className="block text-sm font-semibold text-slate-900">Community</span>}<div className="mt-1 flex flex-wrap items-center gap-2 text-xs font-medium text-slate-400">{authorUsername ? <Link to={`/profile/${encodeURIComponent(authorUsername)}`} className="font-medium text-slate-500 hover:text-violet-700">{authorLabel}</Link> : <span>{authorLabel}</span>}<span>•</span><time>{formattedDate}</time></div></div></div>
            <span className="rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.1em]" style={flair ? { backgroundColor: `${flair.color}20`, color: flair.color } : undefined}>{typeLabel}</span>
          </div>
          <h1 className="mt-6 break-words text-3xl font-bold leading-[1.16] tracking-[-0.04em] text-slate-950 sm:text-[2.15rem]">{data.title}</h1>
