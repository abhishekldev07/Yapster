      ...
      {user ? (
        <>
          ...
          <form onSubmit={handleSubmit} className="mb-6 rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm">
            <textarea value={newCommentText} onChange={(event) => setNewCommentText(event.target.value)} disabled={!canComment} className="min-h-24 w-full resize-y rounded-xl border-0 bg-transparent px-1 py-1 text-sm font-normal leading-6 text-slate-800 placeholder:text-slate-400 outline-none disabled:cursor-not-allowed disabled:opacity-50" placeholder={canComment ? "Add to the discussion..." : "Join this community to comment"} rows={3} />
            <div className="mt-2 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3"><p className="text-[11px] font-medium text-slate-400">Keep it relevant and constructive. Mention someone with @username.</p><button type="submit" disabled={isPending || !canComment || !newCommentText.trim()} className="yapster-button yapster-button--primary min-h-9 px-3.5 py-0 text-xs disabled:cursor-not-allowed disabled:opacity-45">{isPending ? "Posting..." : "Comment"}</button></div>
