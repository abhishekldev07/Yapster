...
export const CommunityMembersAdminPage = () => {
  ...
  if (!user || !canManage) return <main className="pb-16 pt-7"><div className="mx-auto max-w-3xl px-4 sm:px-6"><div className="yapster-card p-9 text-center"><p className="text-[11px] font-semibold uppercase tracking-[0.13em] text-slate-400">Member safety</p><h1 className="mt-2 text-2xl font-bold text-slate-950">Moderator access required</h1><p className="mt-2 text-sm text-slate-500">Only the owner and active moderators can remove members.</p><Link to={`/community/${communityId}`} className="yapster-button yapster-button--primary mt-5">Back to community</Link></div></div></main>;
  ...
  return (
    <main className="pb-16 pt-7 max-[760px]:pb-24 max-[760px]:pt-4">
      ...
      {removeTarget && <div className="fixed inset-0 z-[100] grid place-items-center bg-black/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true"><form onSubmit={(event) => { event.preventDefault(); removeMutation.mutate(); }} className="w-full max-w-lg rounded-[22px] border border-slate-200 bg-white p-5 shadow-2xl sm:p-6"><p className="text-[11px] font-semibold uppercase tracking-[0.13em] text-slate-400">Remove member</p><h2 className="mt-2 text-xl font-bold text-slate-950">Remove {removeTarget.name}?</h2><p className="mt-2 text-sm leading-6 text-slate-500">Tell them why they are being removed. This reason will be included in their notification.</p><label className="mt-5 block text-sm font-semibold text-slate-700">Reason<textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={4} maxLength={300} autoFocus placeholder="e.g. Repeated spam after moderator warnings" className="mt-2 w-full resize-y rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm font-normal text-slate-800 outline-none focus:border-violet-300 focus:bg-white" /></label><div className="mt-1 text-right text-[10px] font-medium text-slate-400">{reason.length}/300</div>{removeMutation.error && <div className="mt-3 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{getFriendlyErrorMessage(removeMutation.error, "Member could not be removed.")}</div>}<div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => { setRemoveTarget(null); setReason(""); }} className="yapster-button yapster-button--ghost">Cancel</button><button type="submit" disabled={!reason.trim() || removeMutation.isPending} className="yapster-button yapster-button--primary disabled:opacity-50">{removeMutation.isPending ? "Removing..." : "Remove member"}</button></div></form></div>}
    </main>
  );
};