...
const fetchInbox = async (userId: string): Promise<InboxConversation[]> => {
  ...
  const stateMap = new Map<number, ConversationState>((states ?? []).map((row) => [Number(row.conversation_id), {
    conversation_id: Number(row.conversation_id), hidden_at: row.hidden_at ?? null, pinned_at: row.pinned_at ?? null,
    manually_unread: Boolean(row.manually_unread), cleared_at: row.cleared_at ?? null,
  }]));
  ...
  return rows.map((row) => {
    ...
    return { ...row, state, partner: profileMap.get(partnerId) ?? null, latestMessage: latestMap.get(row.id) ?? null, unreadCount: actualUnread || (state.manually_unread ? 1 : 0) };
  }).sort((a, b) => {
    const aPinned = timeValue(a.state.pinned_at); const bPinned = timeValue(b.state.pinned_at);
...
export const MessagesPage = () => {
  ...
  const pinMutation = useMutation({ mutationFn: async (conversation: InboxConversation) => { if (!user) throw new Error("Sign in first."); await updateConversationState(conversation.id, user.id, { pinned_at: conversation.state.pinned_at ? null : new Date().toISOString() }); }, onSuccess: () => queryClient.invalidateQueries({ queryKey: ["message-inbox", user?.id] }) });
  const markUnreadMutation = useMutation({ mutationFn: async (conversationId: number) => { if (!user) throw new Error("Sign in first."); await updateConversationState(conversationId, user.id, { manually_unread: true }); }, onSuccess: async (_, conversationId) => { await Promise.all([queryClient.invalidateQueries({ queryKey: ["message-inbox", user?.id] }), queryClient.invalidateQueries({ queryKey: ["messages-unread-count", user?.id] })]); if (activeConversationId === conversationId) navigate("/messages"); } });
  ...
  const chatActionMutation = useMutation({
    mutationFn: async (action: PendingChatAction) => {
      ...
      if (action.kind === "delete") return updateConversationState(action.conversationId, user.id, { hidden_at: new Date().toISOString(), manually_unread: false });
  ...
  const reportMutation = useMutation({ mutationFn: async () => { if (!user || !reportTarget) throw new Error("Choose a message to report."); const { error } = await supabase.from("direct_message_reports").insert({ reporter_id: user.id, message_id: reportTarget.id, reason: reportReason, details: reportDetails.trim() }); if (error) throw new Error(error.message); }, onSuccess: () => { setReportTarget(null); setReportReason("spam"); setReportDetails(""); } });
  ...
  const renderMessageActions = (message: MessageRow, own: boolean) => {
    ...
    return <div className={`yapster-message-hover-actions relative flex items-center gap-0.5 ${open || reactionOpen ? "is-open" : ""}`} data-chat-popover>
      ...
      {reactionOpen && !interactionBlocked && <div className={`absolute bottom-full z-40 mb-2 flex gap-1 rounded-full border border-slate-200 bg-white p-1.5 shadow-lg ${own ? "right-0" : "left-0"}`}>{QUICK_REACTIONS.map((emoji) => <button key={emoji} type="button" onClick={() => reactionMutation.mutate({ messageId: message.id, emoji })} className="grid h-8 w-8 place-items-center rounded-full text-lg transition hover:bg-slate-100 hover:scale-110" aria-label={`React ${emoji}`}>{emoji}</button>)}</div>}
      {open && <div className={`yapster-popover-menu absolute top-8 z-40 overflow-hidden rounded-xl p-1 ${own ? "right-0" : "left-0"}`}>
        {own ? <>
          {!interactionBlocked && <button type="button" onClick={() => { setEditingMessageId(message.id); setEditText(message.body); setOpenMessageMenuId(null); }}>Edit message</button>}
  ...
  return (
    <main className="pb-16 pt-7 max-[760px]:pb-24 max-[760px]:pt-4">
        ...
        <div className="grid min-h-[620px] overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm lg:grid-cols-[330px_minmax(0,1fr)]">
          <aside className={`border-slate-200 bg-slate-50/60 lg:border-r ${activeConversationId ? "hidden lg:block" : "block"}`}>
            ...
            {inboxQuery.isLoading ? <div className="space-y-2 p-3">{[0,1,2].map((item) => <div key={item} className="h-20 animate-pulse rounded-2xl bg-slate-100" />)}</div> : inboxQuery.error ? <p className="p-5 text-sm text-red-600">Inbox could not be loaded.</p> : conversations.length ? (
              <div className="max-h-[555px] overflow-y-auto p-2.5">{conversations.map((conversation) => {
                ...
                return <div key={conversation.id} className={`yapster-inbox-row group relative mb-1 rounded-2xl transition ${selected ? "is-selected bg-slate-200/60" : "hover:bg-white"}`} data-chat-popover>
                  ...
                  {menuOpen && <div className="yapster-popover-menu overflow-hidden rounded-xl p-1">
                    ...
                    <button type="button" onClick={() => { setPendingChatAction({ kind: "block", conversationId: conversation.id, partnerId: conversation.partner?.id, partnerName: name }); setOpenInboxMenuId(null); }}><span className="inline-flex items-center gap-2"><BlockIcon />Block</span></button>
          ...
          <section className={`${activeConversationId ? "flex" : "hidden lg:flex"} min-w-0 flex-col bg-white`}>
            {!activeConversationId ? <div className="m-auto max-w-sm p-8 text-center"><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-violet-50 text-violet-700"><ChatIcon /></div><h2 className="mt-4 text-xl font-bold text-slate-950">Choose a conversation</h2><p className="mt-2 text-sm leading-6 text-slate-500">Select someone from your inbox, or start a new chat from their profile.</p></div> : !activeConversation && !inboxQuery.isLoading ? <div className="m-auto max-w-sm p-8 text-center"><h2 className="text-xl font-bold text-slate-950">Conversation unavailable</h2><p className="mt-2 text-sm text-slate-500">It may have been deleted from your inbox or you may not have access.</p><button type="button" onClick={() => navigate("/messages")} className="yapster-button yapster-button--primary mt-5">Back to inbox</button></div> : <>
              <div className="flex items-center gap-3 border-b border-slate-200 px-4 py-3.5 sm:px-5"><button type="button" onClick={() => navigate("/messages")} className="grid h-9 w-9 place-items-center rounded-xl border border-slate-200 text-slate-500 lg:hidden" aria-label="Back to inbox">←</button>{partnerAvatar ? <img src={partnerAvatar} alt="" className="h-10 w-10 rounded-xl object-cover ring-1 ring-slate-200" /> : <span className="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-orange-100 via-pink-100 to-violet-100 text-sm font-semibold text-violet-700">{partnerName.slice(0,1).toUpperCase()}</span>}<div className="min-w-0 flex-1"><strong className="block truncate text-sm font-semibold text-slate-950">{partnerName}</strong>{partnerUsername && <Link to={`/profile/${encodeURIComponent(partnerUsername)}`} className="text-xs font-medium text-violet-700">@{partnerUsername}</Link>}</div></div>

              {interactionBlocked && <div className="yapster-chat-blocked-banner flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-100 px-4 py-3 sm:px-5"><div className="flex min-w-0 items-center gap-2.5 text-xs font-medium text-slate-600"><BlockIcon /><span>{blockQuery.data?.blockedByMe ? "You blocked this person. Unblock to interact again." : "This person blocked you. You can view this chat, but interactions are unavailable."}</span></div>{blockQuery.data?.blockedByMe && <button type="button" onClick={() => unblockMutation.mutate()} disabled={unblockMutation.isPending} className="yapster-button yapster-button--ghost inline-flex min-h-8 items-center gap-2 px-3 py-0 text-xs disabled:opacity-50"><BlockIcon />{unblockMutation.isPending ? "Unblocking..." : "Unblock"}</button>}</div>}

              <div className="flex-1 overflow-y-auto bg-slate-50/40 px-4 py-5 sm:px-6">
                {messagesQuery.isLoading ? <p className="text-center text-sm text-slate-400">Loading messages...</p> : messagesQuery.error ? <p className="text-center text-sm text-red-600">Messages could not be loaded.</p> : messages.length ? <div className="space-y-3">{messages.map((message) => {
                  ...
                  return <div key={message.id} className={`yapster-message-row group flex items-center gap-2 ${own ? "justify-end" : "justify-start"}`}>
                    ...
                    <div className="max-w-[82%] sm:max-w-[72%]">
                      ...
                      {!deleted && groupedReactions(message.id).length > 0 && <div className={`relative z-10 -mt-1 flex flex-wrap gap-1 ${own ? "justify-end pr-2" : "justify-start pl-2"}`}>{groupedReactions(message.id).map(([emoji, rows]) => { const mine = rows.some((row) => row.user_id === user.id); return <button key={emoji} type="button" disabled={interactionBlocked} onClick={() => reactionMutation.mutate({ messageId: message.id, emoji })} className={`rounded-full border px-1.5 py-0.5 text-xs transition disabled:cursor-default ${mine ? "border-violet-300 bg-violet-50" : "border-slate-200 bg-white"}`}>{emoji}{rows.length > 1 ? <span className="ml-1 text-[10px] text-slate-400">{rows.length}</span> : null}</button>; })}</div>}
                    </div>
                    {!own && renderMessageActions(message, false)}
                  </div>;
                })}</div> : <div className="mx-auto mt-24 max-w-sm text-center"><p className="font-semibold text-slate-800">Start the conversation</p><p className="mt-1 text-sm text-slate-400">Send a message to {partnerName}.</p></div>}
              </div>
      ...
      <ConfirmDialog open={!!pendingChatAction} title={pendingChatAction?.kind === "block" ? `Block ${pendingChatAction.partnerName}?` : pendingChatAction?.kind === "clear" ? "Clear this chat?" : "Delete chat from your inbox?"} description={pendingChatAction?.kind === "block" ? "The conversation will stay visible, but neither of you can send messages or react until you unblock them." : pendingChatAction?.kind === "clear" ? "Existing messages will be hidden for you. The conversation stays in your Inbox and new messages will still appear." : "This hides the conversation from your Inbox. It does not delete the other person’s history. Opening Message from their profile can restore it."} confirmLabel={pendingChatAction?.kind === "block" ? "Block" : pendingChatAction?.kind === "clear" ? "Clear chat" : "Delete chat"} isPending={chatActionMutation.isPending} onCancel={() => setPendingChatAction(null)} onConfirm={() => pendingChatAction && chatActionMutation.mutate(pendingChatAction)} />
