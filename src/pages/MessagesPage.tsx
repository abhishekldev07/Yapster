...
interface ConversationRow { id: number; user_low: string; user_high: string; created_at: string; last_message_at: string; }
interface ConversationState { conversation_id: number; hidden_at: string | null; pinned_at: string | null; manually_unread: boolean; cleared_at: string | null; }
interface ProfileSummary { id: string; username: string | null; display_name: string | null; avatar_url: string | null; }
interface MessageRow { id: number; conversation_id: number; sender_id: string; body: string; created_at: string; read_at: string | null; edited_at: string | null; deleted_at: string | null; replied_to_message_id: number | null; }
interface ReactionRow { message_id: number; user_id: string; emoji: string; created_at: string; }
interface InboxConversation extends ConversationRow { partner: ProfileSummary | null; latestMessage: MessageRow | null; unreadCount: number; state: ConversationState; }
interface BlockState { blockedByMe: boolean; blockedMe: boolean; }
type ReportReason = "spam" | "harassment" | "hate" | "threat" | "scam" | "sexual" | "other";
type PendingChatAction = { kind: "clear" | "delete"; conversationId: number; partnerName: string };
...
const formatMessageTime = (value: string) => new Date(value).toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" });
const isMessageEditable = (message: MessageRow, now = Date.now()) => {
...
const fetchInbox = async (userId: string): Promise<InboxConversation[]> => {
  ...
  const [{ data: profiles, error: profileError }, { data: messages, error: messageError }, { data: hiddenMessages, error: hiddenError }] = await Promise.all([
  ...
  ((messages ?? []) as MessageRow[]).forEach((message) => {
    ...
    const cutoff = timeValue(state?.cleared_at);
    if (cutoff != null && new Date(message.created_at).getTime() <= cutoff) return;
...
const BlockIcon = () => <svg viewBox="0 0 20 20" className="h-4 w-4 fill-none stroke-current stroke-[1.8]" aria-hidden="true"><circle cx="10" cy="10" r="6.5" /><path d="m5.5 5.5 9 9" strokeLinecap="round" /></svg>;
...
export const MessagesPage = () => {
  ...
  const reactionsQuery = useQuery<ReactionRow[], Error>({ queryKey: ["direct-message-reactions", messageIds.join(",")], queryFn: () => fetchMessageReactions(messageIds), enabled: messageIds.length > 0, retry: false, staleTime: 2_000 });
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setEditClock(Date.now()), 10_000);
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`direct-messages:${activeConversationId}:${user.id}`)
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "direct_message_reactions" }, () => queryClient.invalidateQueries({ queryKey: ["direct-message-reactions"] }))
      .subscribe();
    return () => { void supabase.removeChannel(channel); };
  }, [activeConversationId, queryClient, user]);

  useEffect(() => {
    if (!user || !activeConversationId || !activeConversation?.state.manually_unread) return;
  ...
  useEffect(() => {
    if (!user || !activeConversationId || !messagesQuery.data?.some((message) => message.sender_id !== user.id && !message.read_at)) return;
    void supabase.rpc("mark_direct_conversation_read", { p_conversation_id: activeConversationId }).then(({ error }) => {
      if (!error) {
        queryClient.invalidateQueries({ queryKey: ["direct-messages", activeConversationId] });
        queryClient.invalidateQueries({ queryKey: ["message-inbox", user.id] });
        queryClient.invalidateQueries({ queryKey: ["messages-unread-count", user.id] });
  ...
  const refreshMessages = async () => Promise.all([
    queryClient.invalidateQueries({ queryKey: ["direct-messages"] }),
    queryClient.invalidateQueries({ queryKey: ["direct-message-reactions"] }),
    queryClient.invalidateQueries({ queryKey: ["message-inbox", user?.id] }),
    queryClient.invalidateQueries({ queryKey: ["messages-unread-count", user?.id] }),
    queryClient.invalidateQueries({ queryKey: ["message-block-state", user?.id, partnerId] }),
  ]);

  const sendMutation = useMutation({
    mutationFn: async () => {
      if (!user || !activeConversationId) throw new Error("Open a conversation first.");
      if (blockQuery.data?.blockedByMe || blockQuery.data?.blockedMe) throw new Error("Unblock to interact again.");
  ...
  const editMutation = useMutation({
    mutationFn: async () => {
      ...
      if (blockQuery.data?.blockedByMe || blockQuery.data?.blockedMe) throw new Error("Unblock to interact again.");
      const target = messagesQuery.data?.find((message) => message.id === editingMessageId);
      if (!target || !isMessageEditable(target)) throw new Error("Message update failed.");
    ...
    onError: () => { setEditingMessageId(null); setEditText(""); },
  });
  ...
  const chatActionMutation = useMutation({
    mutationFn: async (action: PendingChatAction) => {
      if (!user) throw new Error("Sign in first.");
      if (action.kind === "clear") return updateConversationState(action.conversationId, user.id, { cleared_at: new Date().toISOString(), hidden_at: null, manually_unread: false });
      return updateConversationState(action.conversationId, user.id, { hidden_at: new Date().toISOString(), manually_unread: false });
    },
    onSuccess: async (_, action) => {
      setPendingChatAction(null);
      await refreshMessages();
      if (action.kind === "delete") navigate("/messages");
    },
  });

  const unblockMutation = useMutation({
    mutationFn: async () => {
      if (!user || !partnerId) throw new Error("This member could not be resolved.");
      const { error } = await supabase.from("user_blocks").delete().eq("blocker_id", user.id).eq("blocked_id", partnerId);
      if (error) throw new Error(error.message);
    },
    onSuccess: async () => { await refreshMessages(); },
  });
  ...
  if (!user) {
    return (
      <main className="yapster-messages-page pb-16 pt-7 max-[760px]:pb-24 max-[760px]:pt-4">
        <div className="yapster-messages-shell mx-auto max-w-2xl px-4 sm:px-6">
          <section className="yapster-card p-10 text-center">
            ...
            <h1 className="mt-5 text-2xl font-bold text-slate-950">Messages</h1>
  ...
  const mutationError = sendMutation.error || editMutation.error || deleteMessageMutation.error || hideMessageMutation.error || pinMutation.error || markUnreadMutation.error || chatActionMutation.error || unblockMutation.error || reactionMutation.error;

  const groupedReactions = (messageId: number) => {
    const groups = new Map<string, ReactionRow[]>();
    (reactionsQuery.data ?? []).filter((reaction) => reaction.message_id === messageId).forEach((reaction) => groups.set(reaction.emoji, [...(groups.get(reaction.emoji) ?? []), reaction]));
  ...
  const renderMessageActions = (message: MessageRow, own: boolean) => {
    ...
    const editable = own && !interactionBlocked && isMessageEditable(message, editClock);

    return (
      <div className={`yapster-message-hover-actions relative flex items-center gap-0.5 ${open || reactionOpen ? "is-open" : ""}`} data-chat-popover>
        {!interactionBlocked && <button type="button" onClick={(event) => { event.stopPropagation(); setReactionPickerMessageId((current) => current === message.id ? null : message.id); setOpenMessageMenuId(null); }} className="grid h-7 w-7 place-items-center rounded-full text-slate-400 transition hover:bg-slate-200 hover:text-slate-700" aria-label="React to message"><SmileIcon /></button>}
        ...
        {reactionOpen && !interactionBlocked && (
          <div className={`absolute bottom-full z-40 mb-2 flex gap-1 rounded-full border border-slate-200 bg-white p-1.5 shadow-lg ${own ? "right-0" : "left-0"}`}>
            {QUICK_REACTIONS.map((emoji) => <button key={emoji} type="button" onClick={() => reactionMutation.mutate({ messageId: message.id, emoji })} className="grid h-8 w-8 place-items-center rounded-full text-lg transition hover:bg-slate-100 hover:scale-110" aria-label={`React ${emoji}`}>{emoji}</button>)}
          </div>
        )}

        {open && (
          <div className={`yapster-popover-menu absolute top-8 z-40 overflow-hidden rounded-xl p-1 ${own ? "right-0" : "left-0"}`}>
            {own ? <>
              {editable && <button type="button" onClick={() => { setEditingMessageId(message.id); setEditText(message.body); setOpenMessageMenuId(null); }}>Edit message</button>}
              <button type="button" onClick={() => { void copyMessage(message); setOpenMessageMenuId(null); }}>Copy message</button>
              {!interactionBlocked && <button type="button" onClick={() => { setDeleteMessageId(message.id); setOpenMessageMenuId(null); }} className="text-red-600!">Delete message</button>}
            </> : <>
              {!interactionBlocked && <button type="button" onClick={() => { setReplyTarget(message); setOpenMessageMenuId(null); }}>Reply</button>}
              <button type="button" onClick={() => { void copyMessage(message); setOpenMessageMenuId(null); }}>Copy message</button>
              {!interactionBlocked && <button type="button" onClick={() => { setReportTarget(message); setOpenMessageMenuId(null); }}>Report message</button>}
  ...
  return (
    <main className={`yapster-messages-page ${activeConversationId ? "is-thread-open" : ""} pb-16 pt-7 max-[760px]:pb-24 max-[760px]:pt-4`}>
      <div className="yapster-messages-shell mx-auto max-w-[1120px] px-4 sm:px-6">
        <header className="yapster-messages-page-header mb-5 flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-[-0.04em] text-slate-950">Messages</h1>
            <p className="mt-2 text-sm text-slate-500">One-to-one conversations between Yapster members.</p>
          </div>
          {(unreadQuery.data ?? 0) > 0 && <span className="rounded-full bg-violet-50 px-3 py-1.5 text-xs font-semibold text-violet-700">{unreadQuery.data} unread</span>}
        </header>

        <div className="yapster-messages-layout grid min-h-[620px] overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm lg:grid-cols-[330px_minmax(0,1fr)]">
          <aside className={`yapster-messages-inbox border-slate-200 bg-slate-50/60 lg:border-r ${activeConversationId ? "hidden lg:block" : "block"}`}>
            ...
            {inboxQuery.isLoading ? (
              ...
              <div className="max-h-[555px] overflow-y-auto p-2.5">
                {conversations.map((conversation) => {
                  ...
                  return (
                    <div key={conversation.id} className={`yapster-inbox-row group relative mb-1 rounded-2xl transition ${selected ? "is-selected bg-slate-200/60" : "hover:bg-white"}`} data-chat-popover>
                      <button type="button" onClick={() => navigate(`/messages/${conversation.id}`)} className="flex w-full items-center gap-3 rounded-2xl px-3 py-3 pr-10 text-left">
                        ...
                        <span className="min-w-0 flex-1">
                          <span className="flex items-center justify-between gap-2"><strong className="truncate text-sm font-semibold text-slate-900">{name}</strong><time className="shrink-0 text-[10px] font-medium text-slate-400">{formatMessageTime(conversation.latestMessage?.created_at || conversation.last_message_at)}</time></span>
                          <span className="mt-1 flex items-center justify-between gap-2"><span className="truncate text-xs text-slate-500">{preview}</span><span className="flex items-center gap-1.5">{conversation.state.pinned_at && <span className="text-slate-400"><PinIcon /></span>}{conversation.unreadCount > 0 && <span className="grid min-w-5 place-items-center rounded-full bg-violet-600 px-1.5 py-0.5 text-[9px] font-semibold text-white">{conversation.unreadCount > 9 ? "9+" : conversation.unreadCount}</span>}</span></span>
                        </span>
                      </button>
                      ...
                      {menuOpen && (
                        <div className="yapster-popover-menu overflow-hidden rounded-xl p-1">
              ...
              <div className="p-8 text-center">
                ...
                <Link to="/search" className="mt-4 inline-flex text-xs font-semibold text-violet-700">Find people</Link>
              </div>
            )}
          </aside>

          <section className={`yapster-messages-thread ${activeConversationId ? "flex" : "hidden lg:flex"} min-w-0 flex-col bg-white`}>
            {!activeConversationId ? (
              <div className="m-auto max-w-sm p-8 text-center">
                <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-violet-50 text-violet-700"><ChatIcon /></div>
                <h2 className="mt-4 text-xl font-bold text-slate-950">Choose a conversation</h2>
              ...
              <div className="m-auto max-w-sm p-8 text-center">
                <h2 className="text-xl font-bold text-slate-950">Conversation unavailable</h2>
                <p className="mt-2 text-sm text-slate-500">It may have been deleted from your inbox or you may not have access.</p>
              ...
              <div className="flex items-center gap-3 border-b border-slate-200 px-4 py-3.5 sm:px-5">
                ...
                <div className="min-w-0 flex-1">
                  <strong className="block truncate text-sm font-semibold text-slate-950">{partnerName}</strong>
                  {partnerUsername && <Link to={`/profile/${encodeURIComponent(partnerUsername)}`} className="text-xs font-medium text-violet-700">@{partnerUsername}</Link>}
                </div>
              </div>

              {interactionBlocked && (
                <div className="yapster-chat-blocked-banner flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-100 px-4 py-3 sm:px-5">
                  <div className="flex min-w-0 items-center gap-2.5 text-xs font-medium text-slate-600"><BlockIcon /><span>{blockQuery.data?.blockedByMe ? "You blocked this person. Unblock to interact again." : "This person blocked you. You can view this chat, but interactions are unavailable."}</span></div>
                  {blockQuery.data?.blockedByMe && <button type="button" onClick={() => unblockMutation.mutate()} disabled={unblockMutation.isPending} className="yapster-button yapster-button--ghost inline-flex min-h-8 items-center gap-2 px-3 py-0 text-xs disabled:opacity-50"><BlockIcon />{unblockMutation.isPending ? "Unblocking..." : "Unblock"}</button>}
                </div>
              )}

              <div className="yapster-messages-scroll flex-1 overflow-y-auto bg-slate-50/40 px-4 py-5 sm:px-6">
                {messagesQuery.isLoading ? (
                  ...
                  <div className="space-y-3">
                    {messages.map((message) => {
                      ...
                      return (
                        <div key={message.id} className={`yapster-message-row group flex items-center gap-2 ${own ? "justify-end" : "justify-start"}`}>
                          {own && renderMessageActions(message, true)}
                          <div className="max-w-[82%] sm:max-w-[72%]">
                            {editing ? (
                              <form onSubmit={(event) => { event.preventDefault(); editMutation.mutate(); }} className="rounded-2xl border border-violet-200 bg-white p-2 shadow-sm">
                                <textarea value={editText} onChange={(event) => setEditText(event.target.value)} rows={2} maxLength={4000} autoFocus className="w-full resize-y rounded-xl border border-slate-200 bg-slate-50 p-2.5 text-sm font-normal text-slate-800 outline-none focus:border-violet-300" />
                                <div className="mt-2 flex justify-end gap-2"><button type="button" onClick={() => { setEditingMessageId(null); setEditText(""); }} className="text-xs font-medium text-slate-500">Cancel</button><button type="submit" disabled={!editText.trim() || editMutation.isPending} className="rounded-lg bg-violet-600 px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50">{editMutation.isPending ? "Saving..." : "Save"}</button></div>
                              </form>
                            ) : (
                              <div className={`rounded-2xl px-4 py-2.5 shadow-sm ${own ? "bg-violet-600 text-white" : "border border-slate-200 bg-white text-slate-800"}`}>
                                ...
                                <div className="flex items-end gap-2"><p className={`min-w-0 flex-1 whitespace-pre-wrap break-words text-sm leading-6 ${deleted ? "italic opacity-65" : ""}`}>{deleted ? "Message deleted" : message.body}</p><div className={`mb-0.5 flex shrink-0 items-center gap-1 text-[10px] font-medium ${own ? "text-white/70" : "text-slate-400"}`}><span>{formatMessageTime(message.created_at)}</span>{message.edited_at && !deleted && <span>edited</span>}{copiedMessageId === message.id && <span>copied</span>}{own && <span title={message.read_at ? "Read" : "Sent"} className="text-[12px] tracking-[-0.16em]">{message.read_at ? "✓✓" : "✓"}</span>}</div></div>
                              </div>
                            )}

                            {!deleted && groupedReactions(message.id).length > 0 && (
                              <div className={`relative z-10 -mt-1 flex flex-wrap gap-1 ${own ? "justify-end pr-2" : "justify-start pl-2"}`}>
                                {groupedReactions(message.id).map(([emoji, rows]) => {
                                  const mine = rows.some((row) => row.user_id === user.id);
                                  return <button key={emoji} type="button" disabled={interactionBlocked} onClick={() => reactionMutation.mutate({ messageId: message.id, emoji })} className={`rounded-full border px-1.5 py-0.5 text-xs transition disabled:cursor-default ${mine ? "border-violet-300 bg-violet-50" : "border-slate-200 bg-white"}`}>{emoji}{rows.length > 1 ? <span className="ml-1 text-[10px] text-slate-400">{rows.length}</span> : null}</button>;
                                })}
                              </div>
                            )}
                          </div>
              ...
              <form className="yapster-message-composer-shell border-t border-slate-200 bg-white p-3 sm:p-4" onSubmit={(event) => { event.preventDefault(); if (!sendMutation.isPending && composer.trim() && !interactionBlocked) sendMutation.mutate(); }}>
                ...
                <div className="flex items-end gap-2">
                  <div className="relative" data-chat-popover>
                    <button type="button" disabled={interactionBlocked} onClick={() => setEmojiPickerOpen((current) => !current)} className="grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-slate-50 text-slate-500 transition hover:bg-slate-100 hover:text-slate-800 disabled:cursor-not-allowed disabled:opacity-45" aria-label="Add emoji"><SmileIcon /></button>
                    {emojiPickerOpen && !interactionBlocked && <div className="absolute bottom-13 left-0 z-50 grid w-72 grid-cols-8 gap-1.5 rounded-2xl border border-slate-200 bg-white p-3 shadow-xl">{COMPOSER_EMOJIS.map((emoji) => <button key={emoji} type="button" onClick={() => setComposer((current) => `${current}${emoji}`)} className="grid h-8 w-8 place-items-center rounded-lg text-xl transition hover:bg-slate-100 hover:scale-110">{emoji}</button>)}</div>}
                  </div>
                  <textarea value={composer} onChange={(event) => setComposer(event.target.value)} rows={1} maxLength={4000} disabled={interactionBlocked} placeholder={interactionBlocked ? "Unblock to interact again" : `Message ${partnerName}`} className="yapster-message-composer min-h-11 max-h-32 flex-1 resize-y rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3 text-sm font-normal text-slate-800 outline-none transition focus:bg-white disabled:cursor-not-allowed disabled:opacity-50" />
                  <button type="submit" disabled={!composer.trim() || sendMutation.isPending || interactionBlocked} className="yapster-button yapster-button--primary min-h-11 disabled:cursor-not-allowed disabled:opacity-50">{sendMutation.isPending ? "Sending..." : "Send"}</button>
                </div>
              </form>
            </>}
          </section>
      ...
      <ConfirmDialog open={!!pendingChatAction} title={pendingChatAction?.kind === "clear" ? "Clear this chat?" : "Delete chat from your inbox?"} description={pendingChatAction?.kind === "clear" ? "Existing messages will be hidden for you. The conversation stays in your Inbox and new messages will still appear." : "This hides the conversation from your Inbox. It does not delete the other person’s history. Opening Message from their profile can restore it."} confirmLabel={pendingChatAction?.kind === "clear" ? "Clear chat" : "Delete chat"} isPending={chatActionMutation.isPending} onCancel={() => setPendingChatAction(null)} onConfirm={() => pendingChatAction && chatActionMutation.mutate(pendingChatAction)} />

      {reportTarget && (
        <div className="fixed inset-0 z-[100] grid place-items-center bg-black/55 p-4" role="dialog" aria-modal="true" aria-label="Report message">
          <form onSubmit={(event) => { event.preventDefault(); reportMutation.mutate(); }} className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
            <div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-bold text-slate-950">Report message</h2><p className="mt-1 text-sm leading-5 text-slate-500">This report is stored for Yapster administration to review.</p></div><button type="button" onClick={() => { setReportTarget(null); setReportDetails(""); }} className="text-xl text-slate-400 hover:text-slate-700" aria-label="Close report dialog">×</button></div>
            ...
            <label className="mt-4 block text-sm font-semibold text-slate-700">Details <span className="font-normal text-slate-400">· optional</span><textarea value={reportDetails} onChange={(event) => setReportDetails(event.target.value)} maxLength={1200} rows={4} placeholder="Add context for the administrator..." className="mt-2 w-full resize-y rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm font-normal leading-6 text-slate-800 outline-none focus:border-violet-300" /></label>
            {reportMutation.error && <p className="mt-3 text-xs text-red-600">{getFriendlyErrorMessage(reportMutation.error, "This message could not be reported. It may already be reported by you.")}</p>}
            <div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => { setReportTarget(null); setReportDetails(""); }} className="yapster-button yapster-button--ghost">Cancel</button><button type="submit" disabled={reportMutation.isPending} className="yapster-button yapster-button--primary disabled:opacity-50">{reportMutation.isPending ? "Reporting..." : "Submit report"}</button></div>
          </form>
        </div>
      )}
    </main>
