...
const fetchInbox = async (userId: string): Promise<InboxConversation[]> => {
  ...
  return rows.map((row) => {
    ...
    if (aPinned != null || bPinned != null) {
      if (aPinned == null) return 1; if (bPinned == null) return -1; if (aPinned !== bPinned) return bPinned - aPinned;
    }
...
const fetchBlockState = async (userId: string, otherUserId: string): Promise<BlockState> => {
  ...
  if (error) throw new Error(error.message);
  return { blockedByMe: (data ?? []).some((row) => row.blocker_id === userId && row.blocked_id === otherUserId), blockedMe: (data ?? []).some((row) => row.blocker_id === otherUserId && row.blocked_id === userId) };
};
...
export const MessagesPage = () => {
  ...
  const blockQuery = useQuery<BlockState>({ queryKey: ["message-block-state", user?.id, partnerId], queryFn: () => user && partnerId ? fetchBlockState(user.id, partnerId) : { blockedByMe: false, blockedMe: false }, enabled: !!user && !!partnerId, retry: false, staleTime: 5_000 });
  ...
  useEffect(() => {
    const closePopovers = (event: PointerEvent) => {
      ...
      setOpenInboxMenuId(null); setOpenMessageMenuId(null); setReactionPickerMessageId(null); setEmojiPickerOpen(false);
    };
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`direct-messages:${activeConversationId}:${user.id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "direct_messages", filter: `conversation_id=eq.${activeConversationId}` }, () => {
        queryClient.invalidateQueries({ queryKey: ["direct-messages", activeConversationId] }); queryClient.invalidateQueries({ queryKey: ["message-inbox", user.id] }); queryClient.invalidateQueries({ queryKey: ["messages-unread-count", user.id] });
      })
  ...
  useEffect(() => {
    ...
    void updateConversationState(activeConversationId, user.id, { manually_unread: false }).then(() => {
      queryClient.invalidateQueries({ queryKey: ["message-inbox", user.id] }); queryClient.invalidateQueries({ queryKey: ["messages-unread-count", user.id] });
    });
  ...
  useEffect(() => {
    ...
    void supabase.rpc("mark_direct_conversation_read", { p_conversation_id: activeConversationId }).then(({ error }) => {
      if (!error) { queryClient.invalidateQueries({ queryKey: ["direct-messages", activeConversationId] }); queryClient.invalidateQueries({ queryKey: ["message-inbox", user.id] }); queryClient.invalidateQueries({ queryKey: ["messages-unread-count", user.id] }); }
    });
  }, [activeConversationId, messagesQuery.data, queryClient, user]);

  const refreshMessages = async () => Promise.all([
    queryClient.invalidateQueries({ queryKey: ["direct-messages"] }), queryClient.invalidateQueries({ queryKey: ["direct-message-reactions"] }),
    queryClient.invalidateQueries({ queryKey: ["message-inbox", user?.id] }), queryClient.invalidateQueries({ queryKey: ["messages-unread-count", user?.id] }),
  ]);
  ...
  const sendMutation = useMutation({
    mutationFn: async () => {
      ...
      const body = composer.trim(); if (!body) throw new Error("Write a message first.");
  ...
  const editMutation = useMutation({ mutationFn: async () => { if (!editingMessageId) throw new Error("Choose a message to edit."); const body = editText.trim(); if (!body) throw new Error("Message cannot be empty."); const { error } = await supabase.from("direct_messages").update({ body }).eq("id", editingMessageId); if (error) throw new Error(error.message); }, onSuccess: async () => { setEditingMessageId(null); setEditText(""); await refreshMessages(); } });
  const deleteMessageMutation = useMutation({ mutationFn: async () => { if (!deleteMessageId) throw new Error("Choose a message to delete."); const { error } = await supabase.from("direct_messages").update({ body: "", deleted_at: new Date().toISOString() }).eq("id", deleteMessageId); if (error) throw new Error(error.message); }, onSuccess: async () => { setDeleteMessageId(null); await refreshMessages(); } });
  const hideMessageMutation = useMutation({ mutationFn: async () => { if (!user || !hideMessageId) throw new Error("Choose a message to hide."); const { error } = await supabase.from("direct_message_hidden_messages").insert({ message_id: hideMessageId, user_id: user.id }); if (error && !error.message.toLowerCase().includes("duplicate")) throw new Error(error.message); }, onSuccess: async () => { setHideMessageId(null); await refreshMessages(); } });
  ...
  const chatActionMutation = useMutation({
    mutationFn: async (action: PendingChatAction) => {
      ...
      const { error } = await supabase.from("user_blocks").insert({ blocker_id: user.id, blocked_id: action.partnerId }); if (error) throw new Error(error.message);
    },
    onSuccess: async (_, action) => { setPendingChatAction(null); await refreshMessages(); if (action.kind === "delete" || action.kind === "block") navigate("/messages"); },
  });
  const reactionMutation = useMutation({
    mutationFn: async ({ messageId, emoji }: { messageId: number; emoji: string }) => {
      ...
      if (existing?.emoji === emoji) { const { error } = await supabase.from("direct_message_reactions").delete().eq("message_id", messageId).eq("user_id", user.id); if (error) throw new Error(error.message); }
      else { const { error } = await supabase.from("direct_message_reactions").upsert({ message_id: messageId, user_id: user.id, emoji }, { onConflict: "message_id,user_id" }); if (error) throw new Error(error.message); }
    },
  ...
  const copyMessage = async (message: MessageRow) => { if (message.deleted_at) return; await navigator.clipboard.writeText(message.body); setCopiedMessageId(message.id); window.setTimeout(() => setCopiedMessageId((current) => current === message.id ? null : current), 1200); };
  ...
  const renderMessageActions = (message: MessageRow, own: boolean) => {
    ...
    const open = openMessageMenuId === message.id; const reactionOpen = reactionPickerMessageId === message.id;
    return <div className={`yapster-message-hover-actions relative flex items-center gap-0.5 ${open || reactionOpen ? "is-open" : ""}`} data-chat-popover>
      ...
      {open && <div className={`yapster-popover-menu absolute top-8 z-40 overflow-hidden rounded-xl p-1 ${own ? "right-0" : "left-0"}`}>
        {own ? <><button type="button" onClick={() => { setEditingMessageId(message.id); setEditText(message.body); setOpenMessageMenuId(null); }}>Edit message</button><button type="button" onClick={() => { setDeleteMessageId(message.id); setOpenMessageMenuId(null); }} className="text-red-600!">Delete message</button><button type="button" onClick={() => { void copyMessage(message); setOpenMessageMenuId(null); }}>Copy message</button></> : <><button type="button" onClick={() => { setReplyTarget(message); setOpenMessageMenuId(null); }}>Reply</button><button type="button" onClick={() => { void copyMessage(message); setOpenMessageMenuId(null); }}>Copy message</button><button type="button" onClick={() => { setReportTarget(message); setOpenMessageMenuId(null); }}>Report message</button><button type="button" onClick={() => { setHideMessageId(message.id); setOpenMessageMenuId(null); }} className="text-red-600!">Delete for me</button></>}
      </div>}
    </div>;
  };
            ...
            {!activeConversationId ? <div className="m-auto max-w-sm p-8 text-center"><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-violet-50 text-violet-700"><ChatIcon /></div><h2 className="mt-4 text-xl font-bold text-slate-950">Choose a conversation</h2><p className="mt-2 text-sm leading-6 text-slate-500">Select someone from your inbox, or start a new chat from their profile.</p></div> : !activeConversation && !inboxQuery.isLoading ? <div className="m-auto max-w-sm p-8 text-center"><h2 className="text-xl font-bold text-slate-950">Conversation unavailable</h2><p className="mt-2 text-sm text-slate-500">It may have been deleted from your inbox or you may not have access.</p><button type="button" onClick={() => navigate("/messages")} className="yapster-button yapster-button--primary mt-5">Back to inbox</button></div> : <>
              ...
              <div className="flex-1 overflow-y-auto bg-slate-50/40 px-4 py-5 sm:px-6">
                {messagesQuery.isLoading ? <p className="text-center text-sm text-slate-400">Loading messages...</p> : messagesQuery.error ? <p className="text-center text-sm text-red-600">Messages could not be loaded.</p> : messages.length ? <div className="space-y-3">{messages.map((message) => {
                  const own = message.sender_id === user.id; const deleted = Boolean(message.deleted_at); const editing = editingMessageId === message.id; const parent = message.replied_to_message_id ? messageById.get(message.replied_to_message_id) : null;
