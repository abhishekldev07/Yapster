...
export const CommunityModerationPage = () => {
  ...
  const { data: reports = [], isLoading: reportsLoading, error: reportsError } = useQuery<ReportRow[], Error>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("reports")
        .select("id, reporter_id, community_id, target_type, target_id, reason, details, status, created_at, reviewed_at, reviewed_by, resolution_note")
        .eq("community_id", communityId)
  ...
  const { data: modLog = [], isLoading: logLoading } = useQuery<ModLogRow[], Error>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("moderation_log")
        .select("id, actor_id, action_type, target_type, target_id, note, created_at")
        .eq("community_id", communityId)
        .order("created_at", { ascending: false })
        .limit(100);
      if (error) throw new Error(error.message);
  ...
  if (communityLoading) {
    return <main className="pb-16 pt-7"><div className="mx-auto max-w-5xl px-4 sm:px-6"><div className="yapster-card animate-pulse p-8"><div className="h-8 w-64 rounded bg-slate-100" /><div className="mt-6 h-48 rounded-2xl bg-slate-100" /></div></div></main>;
  }
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-4 text-[11px] font-extrabold uppercase tracking-[0.14em] text-red-500">Trust & safety</p>
        ...
        <div className="mb-5 inline-flex rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
          {([
            ["queue", `Queue · ${queueReports.length}`],
            ["history", "History"],
            ["log", "Mod log"],
          ] as [Tab, string][]).map(([key, label]) => (
            <button key={key} type="button" onClick={() => setTab(key)} className={`rounded-lg px-3.5 py-2 text-xs font-extrabold transition ${tab === key ? "bg-slate-950 text-white" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}>{label}</button>
          ))}
        </div>
        ...
        {tab === "log" ? (
            ...
            {visibleReports.map((report) => {
              ...
              return (
                <article key={report.id} className="yapster-card overflow-hidden">
                  <div className="h-1 bg-gradient-to-r from-red-500 via-orange-500 to-violet-600" />
                  <div className="p-5 sm:p-6">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          <span className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-[0.12em] ${report.status === "dismissed" ? "bg-slate-100 text-slate-500" : report.status === "resolved" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>{report.status}</span>
                          <span className="text-xs font-extrabold text-slate-700">{reasonLabels[report.reason] || report.reason}</span>
                          <span className="text-xs text-slate-400">{report.target_type} #{report.target_id}</span>
                        </div>
                        <p className="mt-2 text-xs text-slate-400">Reported by @{reporter} · {formatDate(report.created_at)}</p>
                      </div>
                    ...
                    <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <h3 className="truncate text-sm font-black text-slate-900">{preview?.title || "Content unavailable"}</h3>
                          <p className="mt-1.5 line-clamp-3 whitespace-pre-wrap text-sm leading-6 text-slate-500">{preview?.body || "This content may already have been removed."}</p>
                        </div>
                        {preview && <Link to={preview.href} className="shrink-0 text-xs font-extrabold text-violet-700 hover:text-violet-800">Open →</Link>}
                      </div>
                    </div>
                    ...
                    {(report.status === "open" || report.status === "reviewing") && (
                      <div className="mt-5 flex flex-wrap justify-end gap-2 border-t border-slate-100 pt-4">
                        <button type="button" disabled={resolveMutation.isPending || removeMutation.isPending} onClick={() => resolveMutation.mutate({ report, status: "dismissed" })} className="yapster-button yapster-button--ghost min-h-9 px-3 py-0 text-xs disabled:opacity-50">Dismiss</button>
                        <button type="button" disabled={resolveMutation.isPending || removeMutation.isPending} onClick={() => resolveMutation.mutate({ report, status: "resolved" })} className="rounded-xl border border-green-200 bg-green-50 px-3.5 py-2 text-xs font-extrabold text-green-700 transition hover:bg-green-100 disabled:opacity-50">Resolve · keep content</button>
                        <button type="button" disabled={!preview || resolveMutation.isPending || removeMutation.isPending} onClick={() => setRemoveTarget(report)} className="rounded-xl bg-red-600 px-3.5 py-2 text-xs font-extrabold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-45">Remove {report.target_type}</button>
                      </div>
                    )}
                  </div>
                </article>
          ...
          <section className="yapster-card p-10 text-center">
            <div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-green-50 text-green-700 ring-1 ring-green-100">
              <svg viewBox="0 0 24 24" className="h-6 w-6 fill-none stroke-current stroke-2" aria-hidden="true"><path d="m5 12.5 4 4L19 7" strokeLinecap="round" strokeLinejoin="round" /></svg>
            </div>
            <h2 className="mt-4 text-xl font-black text-slate-950">{tab === "queue" ? "Queue is clear" : "No report history yet"}</h2>
            <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">{tab === "queue" ? "There are no open reports waiting for this community's moderation team." : "Resolved and dismissed reports will appear here."}</p>
          </section>
        )}
      </div>
