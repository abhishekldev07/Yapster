...
export const CommunitySettingsPage = () => {
  ...
  return (
    <>
              ...
              <div className="mt-5 space-y-3">
                {rulesLoading ? <p className="text-sm text-slate-400">Loading rules...</p> : rules.length ? rules.map((rule, index) => (
                  <div key={rule.id} className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-4">
                    ...
                    <button type="button" onClick={() => setPendingDelete({ type: "rule", id: rule.id, label: rule.title })} className="text-xs font-extrabold text-red-500 hover:text-red-700">Delete</button>
                  </div>
              ...
              <div className="mt-5 space-y-3">
                {flairsLoading ? <p className="text-sm text-slate-400">Loading flairs...</p> : flairs.length ? flairs.map((flair) => (
                  <div key={flair.id} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4">
                    ...
                    <button type="button" onClick={() => setPendingDelete({ type: "flair", id: flair.id, label: flair.name })} className="text-xs font-extrabold text-red-500 hover:text-red-700">Delete</button>
                  </div>
      ...
      <ConfirmDialog
        open={!!pendingDelete}
        title={pendingDelete?.type === "rule" ? "Delete community rule?" : "Delete post flair?"}
        description={pendingDelete ? `“${pendingDelete.label}” will be permanently removed. This cannot be undone.` : ""}
        confirmLabel={pendingDelete?.type === "rule" ? "Delete rule" : "Delete flair"}
        isPending={deletePending}
        onCancel={() => setPendingDelete(null)}
        onConfirm={() => {
          if (!pendingDelete) return;
          if (pendingDelete.type === "rule") deleteRule.mutate(pendingDelete.id);
          else deleteFlair.mutate(pendingDelete.id);
        }}
      />
    </>
  );
