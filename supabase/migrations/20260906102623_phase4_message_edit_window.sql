...
begin
  ...
  if v_body_changed or v_deleted_changed then
    select exists (
      select 1
    ...
    elsif v_body_changed then
      if clock_timestamp() > old.created_at + interval '30 minutes' then
        raise exception 'Message edit window has expired';
      end if;
